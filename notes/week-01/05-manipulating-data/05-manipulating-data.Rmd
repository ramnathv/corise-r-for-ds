...
The `dplyr` package also provides a handy set of modifiers (`contains`, `ends_with`, `where` etc) that let you select columns based on a pattern of names. This is very handy especially when you want to pick multiple columns, but don't want to type each column name separately.
